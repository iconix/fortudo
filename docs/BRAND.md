...
- **Violet = brand + interaction only:** header lockup, active tab (`bg-violet-500/20 text-violet-200 border-violet-400/40`), primary buttons: tint+ style — `bg-violet-500/30 border border-violet-400/60 text-violet-200 hover:bg-violet-500/40` (solid `bg-violet-600` is the approved louder alternate). The Settings scrollbar uses a restrained violet thumb because it is an interaction affordance, not a task-type signal.
