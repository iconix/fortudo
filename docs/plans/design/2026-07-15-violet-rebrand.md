...
- Settings visually distinguishes Activity tracking from the shared Organization domain with a restrained divider and helper copy rather than heavier nested cards. During the initial rollout, Organization remains functionally gated by Activity tracking; behavioral decoupling is explicitly deferred until after rollout. Taxonomy hierarchy is consistently left-aligned. Group cards are compact one-line identities with accessible color-dot names; each category group states its tone family once as quiet inline metadata beside the group name; linked category rows omit repetitive metadata; and independent categories use a quiet inline `custom color` label without a badge surface so every row retains the same height. Shared Settings add actions use the brand violet accent rather than a work-type color.
