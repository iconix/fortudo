...
- Settings taxonomy hierarchy is consistently left-aligned. Group cards are compact one-line identities with accessible color-dot names; each category group states its tone family once as the default; linked category rows omit repetitive metadata; and independent categories use a quiet inline `custom color` label without a badge surface so every row retains the same height.
