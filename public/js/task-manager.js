...
function checkAndAdjustForLockedTasks(taskToAdjust, allScheduledTasks) {
    if (taskToAdjust.type !== 'scheduled' || !taskToAdjust.startDateTime || !taskToAdjust.duration) {
    ...
    let adjustedTask = { ...taskToAdjust }; // Work with a copy

    const sortedLockedTasks = allScheduledTasks
        .filter(t => t.id !== adjustedTask.id && t.type === 'scheduled' && t.locked && t.status !== 'completed')
    ...
    do {
        ...
        if (iterations > MAX_ITERATIONS) {
            logger.warn('checkAndAdjustForLockedTasks: Max iterations reached, potential infinite loop. Returning task as is.', adjustedTask);
        ...
        for (const lockedTask of sortedLockedTasks) {
            if (tasksOverlap(adjustedTask, lockedTask)) {
                logger.debug('checkAndAdjustForLockedTasks: Adjusting task', adjustedTask.description, 'due to locked task', lockedTask.description);
                adjustedTask.startDateTime = lockedTask.endDateTime;
                adjustedTask.endDateTime = calculateEndDateTime(adjustedTask.startDateTime, adjustedTask.duration);
...
const createTaskObject = (taskData) => {
    ...
    if (baseTask.type === 'scheduled') {
        ...
        finalTask = { ...baseTask, id: `sched-${Date.now()}`, startDateTime, endDateTime: calculateEndDateTime(startDateTime, taskData.duration), duration: taskData.duration };
    } else { // unscheduled
        finalTask = { ...baseTask, id: `unsched-${Date.now()}`, priority: taskData.priority || 'medium', estDuration: taskData.estDuration !== undefined && taskData.estDuration !== null ? taskData.estDuration : null }; // Ensure estDuration can be null
    }
    logger.debug('Created task object:', finalTask);
    return finalTask;
};

const finalizeTaskModification = () => { logger.debug('Finalizing task modification (invalidate cache, save)'); invalidateTaskCaches(); saveTasks(tasks); };
...
export function isValidTaskData(description, taskType, duration, startTime, estDuration) {
    ...
    if (taskType === 'scheduled') {
        if (duration === undefined || duration === null || isNaN(duration) || duration < 0) {
            return { isValid: false, reason: 'Duration must be a non-negative number for scheduled tasks.' };
        }
    ...
    if (taskType === 'unscheduled') {
        if (estDuration !== undefined && estDuration !== null && (isNaN(estDuration) || estDuration < 0)) {
...
export function checkOverlap(taskToCompare, existingTasks) {
    ...
    const sortedTasks = [...existingTasks].filter(task => task.type === 'scheduled')
        .sort((a, b) => new Date(a.startDateTime).getTime() - new Date(b.startDateTime).getTime());
...
export function performReschedule(actualTask) {
    if (actualTask.type !== 'scheduled' || !actualTask.startDateTime || !actualTask.endDateTime || typeof actualTask.duration !== 'number') {
        logger.warn('performReschedule: Invalid task, missing datetime fields, or duration not a number', actualTask);
        return;
    }
    ...
    for (const otherTask of otherScheduledTasks) {
        ...
        if (otherTask.locked && otherTask.status !== 'completed' && tasksOverlap(actualTask, otherTask)) {
    ...
    for (const otherTask of otherScheduledTasks) {
        ...
        if (!otherTask.startDateTime || typeof otherTask.duration !== 'number') {
            logger.warn('performReschedule: Skipping invalid task in tasksToShift collection', otherTask);
    ...
    for (const taskToReschedule of tasksToShift) {
        ...
        taskToReschedule.endDateTime = calculateEndDateTime(taskToReschedule.startDateTime, taskToReschedule.duration);
...
export function getSuggestedStartTime() {
    ...
    for (const task of scheduledTasks) {
        ...
        if (!task.startDateTime || !task.endDateTime) {
            logger.warn('getSuggestedStartTime - Skipping task with missing start/endDateTime:', task);
            continue;
        }
        ...
        if (task.status === 'incomplete') {
            ...
            if (!hasTaskAtCurrentTime) {
                if (taskEndMinutesValue < taskStartMinutes) { // Task crosses midnight
                    if (
...
export function addTask(taskData, isResubmissionAfterShiftConfirm = false) {
    logger.debug('addTask called with taskData:', taskData, 'isResubmission:', isResubmissionAfterShiftConfirm);
    ...
    let taskObject = createTaskObject(taskData); // This is the task based on current input (could be original or adjusted)
    ...
    if (taskObject.type === 'scheduled') {
        ...
        if (!isResubmissionAfterShiftConfirm) {
            ...
            const taskAfterLockedCheck = checkAndAdjustForLockedTasks(originalRequestedTimeTask, allCurrentScheduledTasks);
            ...
            if (!isScheduledTask(taskAfterLockedCheck)) {
                ...
                return { success: false, reason: 'Internal error: Invalid task data after locked check.' };
            }

            const wasShiftedByLocked = taskAfterLockedCheck.startDateTime !== originalRequestedTimeTask.startDateTime;

            if (wasShiftedByLocked) {
                logger.info('addTask: Task needs shift due to locked tasks.',
                    { originalStart: originalRequestedTimeTask.startDateTime, newStart: taskAfterLockedCheck.startDateTime });
                ...
                return createOverlapConfirmation(
                    'NEEDS_SHIFT_DUE_TO_LOCKED',
                    {
                        ...
                        adjustedTaskDataForResubmission: {
                            ...taskData,
                            startTime: extractTimeFromDateTime(new Date(taskAfterLockedCheck.startDateTime)),
                            duration: taskAfterLockedCheck.duration
                        }
                    },
                    `The task "${originalRequestedTimeTask.description}" would overlap a locked task if scheduled at ${
                        extractTimeFromDateTime(new Date(originalRequestedTimeTask.startDateTime))
                    }, as requested. Schedule it at ${
                        extractTimeFromDateTime(new Date(taskAfterLockedCheck.startDateTime))
                    } instead?`
                );
            }
        }
        ...
        if (!isScheduledTask(taskObject)) {
            ...
            return { success: false, reason: 'Internal error: Invalid task data after resubmission.' };
        }

        const unlockedOverlappingTasks = checkOverlap(taskObject, allCurrentScheduledTasks.filter(t => t.id !== taskObject.id && !t.locked));
        ...
        if (unlockedOverlappingTasks.length > 0) {
            ...
            return createOverlapConfirmation(
                ...
                `Adding "${taskObject.description}" at ${
                    extractTimeFromDateTime(new Date(taskObject.startDateTime))
        ...
        let currentScheduledForAdd = tasks.filter(isScheduledTask);
...
export function confirmAddTaskAndReschedule(confirmedPayload) {
    ...
    if (!taskToAdd || !taskToAdd.id || !taskToAdd.type) {
        logger.error('confirmAddTaskAndReschedule: Invalid taskObject in payload.', confirmedPayload);
        return { success: false, reason: 'Invalid task data for confirmation.' };
    }

    if (taskToAdd.type === 'scheduled') {
        let currentScheduledConfirm = tasks.filter(isScheduledTask);
...
export function updateTask(index, taskData) {
    ...
    if (updatedProposedDetails.type === 'scheduled') {
        ...
        const today = extractDateFromDateTime(new Date());
        const startTime = taskData.startTime || (existingTask.startDateTime ? extractTimeFromDateTime(new Date(existingTask.startDateTime)) : getCurrentTimeRounded());
        updatedProposedDetails.startDateTime = timeToDateTime(startTime, today);
        updatedProposedDetails.endDateTime = calculateEndDateTime(updatedProposedDetails.startDateTime, updatedProposedDetails.duration);
        originalProposedStartDateTime = updatedProposedDetails.startDateTime;

        const allOtherScheduledTasks = tasks.filter(t => t.type === 'scheduled' && t.id !== existingTask.id);
        const taskAfterLockedCheck = checkAndAdjustForLockedTasks(updatedProposedDetails, allOtherScheduledTasks);

        if (taskAfterLockedCheck.startDateTime && originalProposedStartDateTime && taskAfterLockedCheck.startDateTime !== originalProposedStartDateTime) {
            wasShiftedByLocked = true;
            logger.info('updateTask: Task update shifted due to locked tasks. Original proposed start:', originalProposedStartDateTime, 'New start:', taskAfterLockedCheck.startDateTime);
        }
        updatedProposedDetails = taskAfterLockedCheck;

        // This block should be INSIDE the scheduled type check
        const allOverlappingTasks = checkOverlap(updatedProposedDetails, tasks.filter(t => t.id !== existingTask.id && t.type === 'scheduled'));
        unlockedOverlappingTasks = allOverlappingTasks.filter(t => !t.locked); // Assign to the wider scoped variable

        if (unlockedOverlappingTasks.length > 0) {
            if (wasShiftedByLocked) {
                logger.info('updateTask: Updated task (shifted by locked) overlaps unlocked. Auto-rescheduling.');
                ...
                finalizeTaskModification();
                return { success: true, task: tasks[index], autoRescheduledMessage: 'Task updated. Adjusted for locked tasks and automatically rescheduled others.' };
            } else {
                logger.info('updateTask: Updated task (not shifted by locked) overlaps unlocked. Confirmation needed.');
                return createOverlapConfirmation('UPDATE', { taskIndex: index, updatedTaskObject: updatedProposedDetails }, 'Updating this task may overlap. Reschedule others?');
            }
        }
        ...
        delete updatedProposedDetails.startDateTime; delete updatedProposedDetails.endDateTime; delete updatedProposedDetails.duration;
    }

    const validation = isValidTaskData(
        updatedProposedDetails.description, updatedProposedDetails.type,
        updatedProposedDetails.type === 'scheduled' ? updatedProposedDetails.duration : undefined,
        updatedProposedDetails.type === 'scheduled' && updatedProposedDetails.startDateTime ? extractTimeFromDateTime(new Date(updatedProposedDetails.startDateTime)) : undefined,
        updatedProposedDetails.type === 'unscheduled' ? updatedProposedDetails.estDuration : undefined
    );
    if (!validation.isValid) return { success: false, reason: validation.reason };
    ...
    const RtnMessage = wasShiftedByLocked && unlockedOverlappingTasks.length === 0 ? message : undefined;
...
export function updateUnscheduledTask(taskId, newData) {
    ...
    const validation = isValidTaskData(newData.description, 'unscheduled', undefined, undefined, newData.estDuration);
...
export function confirmCompleteLate(index, newEndTime, newDuration) {
    ...
    if (task.type !== 'scheduled' || !task.startDateTime) {
        return { success: false, reason: 'Cannot confirm late completion for non-scheduled or invalid task.' };
    }
...
export function deleteAllTasks() { if (tasks.length === 0) return { success: true, tasksDeleted: 0 }; const num = tasks.length; updateTaskState([]); return { success: true, tasksDeleted: num }; }
...
export function scheduleUnscheduledTask(taskId, startTime, duration) {
    ...
    let scheduledTasks = tasks.filter(t => t.type === 'scheduled');
...
export function reorderUnscheduledTask(draggedTaskId, targetTaskId) {
    ...
    if (draggedTaskIndex === -1 || targetTaskIndex === -1) {
        logger.warn('Dragged or target task not found for reordering.', { draggedTaskId, targetTaskId });
...
export function unscheduleTask(taskId) {
    ...
    task.estDuration = (formerScheduledDuration !== undefined && formerScheduledDuration !== null) ? formerScheduledDuration : null;
