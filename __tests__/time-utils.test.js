...
// Import common test setup and utils functions
...
describe('Time Utility Functions', () => {
  ...
  describe('Date and Time Formatting', () => {
    ...
    test('getCurrentTimeRounded returns exact time when already at 5-minute interval', () => {
      try {
        // With our mock date set to 2:30 PM, the time should already be a multiple of 5
        expect(fortudo.getCurrentTimeRounded()).toBe('14:30');
      } finally {
        dateSpy.mockRestore();
      }
    });
    ...
    test('getCurrentTimeRounded rounds up to nearest 5 minutes', () => {
      // Use a different time for this test
      dateSpy.mockRestore();
      ...
      try {
        ...
        expect(fortudo.getCurrentTimeRounded()).toBe('14:35');
      } finally {
        dateSpy.mockRestore();
      }
    });
    ...
    test('getFormattedDate returns date in readable format', () => {
      ...
      expect(fortudo.getFormattedDate()).toBe('Wednesday, January 15');
    });

    test('getFormattedTime returns time in 12-hour format with AM/PM', () => {
      // With our mock date set to 2:30 PM
      expect(fortudo.getFormattedTime()).toBe('2:30 PM');
    });
  });
});